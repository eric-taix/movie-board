...
saveAllMoviesJSON(String file, String apiKey) {
  new File(file).readAsString().then(JSON.decode).then((List movies) => movies.forEach((Map movie) {
    ...
    saveMovieJSON(movie['id'], movie['poster_path'], apiKey, baseApi: baseApi, prefix: baseApi); 
  }));
}

saveMovieJSON(int movieId, String posterPath, String apiKey, {String baseApi : 'movie', String prefix: ''}) {
    ...
    .then((HttpClientRequest request) => request.close())
      .then((HttpClientResponse response) {
        response.transform(UTF8.decoder).toList().then((data) {
          ...
          File f = new File('movies/${prefix}${movieId}.json');
...
//  saveMovieJSON(550);
  saveAllMoviesJSON('tv_top_rated.json','360c015ba3d57c3c624687c111f91c27');
}